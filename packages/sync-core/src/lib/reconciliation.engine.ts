...
export class ClientReconciliationEngine {
  ...
  constructor() {
    ...
    // NOTE: No platform APIs here. sync-core is pure logic.
    // Platform-specific background detection (AppState on RN, visibilitychange on web)
    // must be wired externally by the adapter layer via onAppStateChange().
  }

  /**
   * Called by the platform adapter when the app transitions between
   * foreground/background. On React Native: wire to AppState.addEventListener.
   * On Web: wire to document.visibilitychange.
   * sync-core itself imports NOTHING platform-specific.
   */
  public onAppStateChange(nextState: 'active' | 'background' | 'inactive') {
    if (nextState === 'background' || nextState === 'inactive') {
      this.dispatchAction({ type: 'CONTINUITY_TRANSITION', state: 'DEGRADED' });
    } else if (nextState === 'active') {
      // Coming back from background — treat as temporal break requiring revalidation
